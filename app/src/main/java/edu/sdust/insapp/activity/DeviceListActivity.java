...
public class DeviceListActivity extends AppCompatActivity implements AbsListView.OnScrollListener {
    ...
    protected void onResume() {
        ...
        if(firstPosition!=0&&top!=0){
            deviceList.setSelectionFromTop(firstPosition, top);
        }
        ...
        adapter  = new SimpleAdapter(this, devices, R.layout.device_list_child, deviceFrom, deviceTo);
        deviceList.setAdapter(adapter);
        deviceList.setSelection(firstPosition);
...
//                }
                ...
                if("设备".equals(text)){
//                    id = i+1;
                    intent.setClass(DeviceListActivity.this, ProblemDetailActivity.class);
                }
                else if("非设备".equals(text)){
//                    id = i+1 - count;
                    intent.setClass(DeviceListActivity.this, NonProblemDetailActivity.class);
                }
//                db.close();
                Log.i("test", ""+id);
                bundle.putInt("id", id);
                    ...
                    public void run() {
                        try {
                            ...
                            if("设备".equals(selectText)){
                                intent.setClass(DeviceListActivity.this, ProblemRegisterActivity.class);

                            }
                            else if("非设备".equals(selectText)){
                                intent.setClass(DeviceListActivity.this, NonProblemRegisterActivity.class);
                            }
                            startActivityForResult(intent, 0);
                        } catch (Exception e) {
                            e.printStackTrace();
...
//初始化tab页
    private void initView() {
        ...
        deviceListView = mInflater.inflate(R.layout.device_list, null);
...
//获取问题列表
    /*public List<Map<String, String>> getData(){
        ...
        db = helper.getReadableDatabase();
        String device_sql = "select _id, deviceId, deviceName from "+DbConstant.DEVICE_FAULT_TABLE;
        Cursor cursor = db.rawQuery(device_sql, null);
        int count = 0;

        while(cursor.moveToNext()){
            Map<String, String> map = new HashMap<>();
            String deviceId = cursor.getString(cursor.getColumnIndex("deviceName"));
    ...
    private List<Map<String, String>> getDevices(){
        ...
        db = helper.getReadableDatabase();
        String sel_sql = "select deviceTag, status, deviceId from "+ DbConstant.DEVICE_TABLE + " order by countId asc";
...
//问题列表统计框
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if(scrollState== AbsListView.OnScrollListener.SCROLL_STATE_IDLE){
            firstPosition=deviceList.getFirstVisiblePosition();
        }
        View v=deviceList.getChildAt(0);
        top=v.getTop();
